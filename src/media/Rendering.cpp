...
void Render_DrawQuad_Colour(QuadPoint<int> center, QuadPoint<unsigned int> size, MColour colour)
{
	SDL_Rect rect = { center.x - ((int)size.x / 2), center.y - ((int)size.y / 2), (int)size.x, (int)size.y };
...
{
	SDL_Rect dst_rect = { center.x - ((int)size.x / 2), center.y - ((int)size.y / 2), (int)size.x, (int)size.y };
...
MTexture* Render_CreateTexture_Text(std::string text, FontID font_id, MFontParameters params)
{
	...
	if (font == NULL)
		LOG_ERROR("Attempted to index nonexistent font. Font index: " + std::to_string((int)font_id));
